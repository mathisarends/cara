"""Sonos audio output adapter.

Sonos players cannot accept raw audio bytes; they fetch media from a URL over
the local network. This adapter therefore serves each WAV clip from a small
in-process HTTP server, tells the speaker to play that URL via SoCo, and polls
the transport state until playback finishes.
"""
...
class SonosAudioPlayer(AudioPlayer):
    ...
    def _resolve_device(self) -> SoCo:
        ...
        if self._speaker_name:
            device = soco.discovery.by_name(self._speaker_name)
            if device is None:
